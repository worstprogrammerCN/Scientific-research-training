...
parser.add_argument("--mat_dir", type=str, default="validation/GT/level0/", help="Json Directory")
parser.add_argument("--image_dir", type=str, default=None, help="Json Directory")
args = parser.parse_args()

if args.image_dir == None:
	args.image_dir = args.mat_dir + "../DRAWING_GT/"
...
class ImageToText(object):
	def __init__(self, items):
		...
		# self.vehicleTotalNum = { vi : len([item for item in self.items if item.category == vi]) for vi in self.vehicle }
		self.plantNum = { p : len([item for item in self.items if item.category == p]) for p in self.plants }
		
		self.hasVehicle = len([item for item in self.items if item.category in self.vehicle]) > 0
	...
	def getDistantView(self):
		...
		texts = []
		if numMountain >= 1: # 有山则统一用mountains复数。
	...
	def setDir(self, this, that):
		"""
			设置this指向that的方向。该方向为direc。
			that在this的direc方向。比如direc为right，则that在this的右边。
		"""
	...
	def getFollowDescription(self, follows, item, wholeDir = None):
		"""
			描述一组动物用。@code{item}是第一个参照物，后面的物体都以前面的物体为参照物。
			比如，@code{item}是@code{follows[0]}的参照物，@code{follows[0]}是@code{follows[1]}的参照物。依次类推。
		"""
		texts = []
		formerItem = item
...
def writeHTML(texts):
	...
	filePath = args.image_dir + "\L0_sample%d.png " # filePath = args.image_dir + "\%d.png
...
def main():
	...
	for num in range(1, 100):
		...
		items = Util.readMat(num, args.mat_dir)
		print([item.category for item in items])
		if items == None:
			continue
		print(num)
