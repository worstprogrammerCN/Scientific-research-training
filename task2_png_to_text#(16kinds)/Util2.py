...
instance = ["bird", "cloud", "sun"] + \
           ["bench", "bus", "car", "house", "tree"] + \
           ["butterfly", "cat", "chicken", "cow", "dog", "duck", "people", "sheep"] + \
           ["road", "grass"]
...
class Position(object):
    ...
    def isHorizontalTo(self, position):
        return self.top == position.top
...
class Item(object):
    ...
    def isFrontOf(self, item):
        return (self.isBottomEdgeBelow(item) or self.center.isFrontOf(item.center) and self.isInteractWith(
            item)) and self.isVerticalTo(item)
    ...
    def findNearestItem(self, items, filter, isFirstLayer=True):
        ...
        dictDirs = {
            "on": self.isOn, "in front of": self.isFrontOf, "on the right front of": self.isRightFrontOf,
            "on the left front of": self.isLeftFrontOf, \
            "on the right of": self.isRightOf, "on the left of": self.isLeftOf,
            "on the left behind of": self.isLeftBehind, \
...
def getItemCategory(class_gt, instance_gt, zIndex):
